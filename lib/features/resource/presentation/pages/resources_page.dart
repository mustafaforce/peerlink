...
class _ResourcesContentState extends State<_ResourcesContent> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.of(context).pushNamed(AppRouter.favoriteResources),
          ),
        ],
      ),
      body: Column(
          ...
          Expanded(
            child: BlocBuilder<ResourceCubit, ResourceState>(
              builder: (context, state) {
                if (state.status == ResourceStatus.loading && state.resources.isEmpty) {
                  return const Center(child: CircularProgressIndicator());
                }

                if (state.status == ResourceStatus.failure && state.resources.isEmpty) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          onPressed: () => context.read<ResourceCubit>().loadResources(refresh: true),
                ...
                return RefreshIndicator(
                  onRefresh: () => context.read<ResourceCubit>().loadResources(refresh: true),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ResourceCard(
                          ...
                          onFavorite: () {
                            ...
                            if (isFav) {
                              context.read<ResourceCubit>().removeFromFavorites(resource['id']);
                            } else {
                              context.read<ResourceCubit>().addToFavorites(resource['id']);
                            }
                          },
                          onDownload: () {
                            context.read<ResourceCubit>().downloadResource(resource['id']);
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.of(context).pushNamed(AppRouter.uploadResource),
  ...
  void _showFilterSheet(BuildContext context) {
    showModalBottomSheet(
      context: context,
      builder: (context) => const _ResourceFilterSheet(),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text('Filter Resources', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          TextField(
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                if (_institutionController.text.isNotEmpty) filter['institution'] = _institutionController.text;
                if (_departmentController.text.isNotEmpty) filter['department'] = _departmentController.text;
                if (_courseController.text.isNotEmpty) filter['course'] = _courseController.text;
                if (_subjectController.text.isNotEmpty) filter['subject'] = _subjectController.text;
